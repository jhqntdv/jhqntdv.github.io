...
2. **Jump Processes**: Models like the Merton Jump-Diffusion (MJD) and Kou Double-Exponential. These explicitly model the discrete jumps in asset prices. Jumps are particularly applicable in the credit market (e.g., default events, sudden changes in credit spreads). While pure Jump-Diffusion models (MJD) are rarely used exclusively, their concepts are embedded in both trading desks and risk management. For example, hybrid frameworks like the Stochastic Volatility with Jumps (SVJ) model are used to accurately price derivatives by capturing both volatility dynamics and the market's implied volatility smile/skew. In risk management, one can see the jump component in Value-at-Risk (VaR) measurement.

## Poisson Process
The **Poisson Distribution** describes the number of times an event occurs within a unit of time, with the core parameter being the intensity $\lambda$. Given a time interval of length $t$, the number of events $N_t$ in a Poisson process satisfies:

$$\mathbb{P}(N_t = k) = \frac{(\lambda t)^k e^{-\lambda t}}{k!}$$

The Poisson process has three main characteristics: 
- events are independent
- the probability of an event occurring in a unit of time is proportional to the interval length
...
Where:
- $N_t$: The Poisson process, which dictates whether a jump occurs.
- $J$: The jump magnitude, usually assumed to be log-normally distributed.
- $S_{t^-}$: The stock price just before the jump.
- $(J - 1)\, dN_t$: The multiplicative change in the asset's value when a jump occurs.
...
- Equity-to-Credit Linkage: The MJD framework links the possibility of a stock price jump to a default event that results in a sudden change in the equity value portion of the convertible.

### Trinomial lattice (tree) model
The convertible bond pricing under MJD is implemented through a trinomial lattice (tree) as it captures early exercise features and the mechanism of default risk. For the stock tree, each node can transition to three states with derived probabilities under the risk-neutral framework: the price going up $p_u$, the price going down $p_d$, and the price being hit by a credit event $p_{\lambda _D}$. The payoff of the convertible bond at each node is also modeled accordingly to reflect stock movements as well as the issuer's and holder's optimal decisions (conversion, redemption, callable, puttable, etc.).
...
- Recovery Rate $\eta$: Assumed percentage of par value (or market value) recovered in the event of default.

- Default Size $1 - \theta$: The assumed percentage drop in stock price upon default. Setting $\theta = 1$ means no jumps.